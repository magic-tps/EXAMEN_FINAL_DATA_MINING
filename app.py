...
Pclass = st.sidebar.selectbox("Clase del pasajero", [1, 2, 3])
Sex = st.sidebar.selectbox("Sexo", ["Mujer", "Hombre"])
Age = st.sidebar.number_input("Edad", min_value=0, value=30)
SibSp = st.sidebar.number_input("Número de hermanos/esposos a bordo", min_value=0, value=0)
Parch = st.sidebar.number_input("Número de padres/hijos a bordo", min_value=0, value=0)
Fare = st.sidebar.number_input("Tarifa del billete", min_value=0.0, value=7.25)
